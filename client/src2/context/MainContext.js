...
const MainContextProvider=(props)=>{
    ...
    const [reloadAllTableTemplate,setReloadAllTableTemplate] = React.useState(true)
    const [allTableTemplate,setAllTableTemplate]=React.useState(null)
    ...
    React.useEffect(()=>{
        //console.log('reloadTableTemplate')
        //console.log(reloadBasicData)
        if(reloadAllTableTemplate){
            
            if(localStorage.getItem('shopauthorization')&&
               localStorage.getItem('userauthorization')
               //to ensure user&shop token in place before request to server
            ){
                axios.post(`/tabletemplate/getcustom`,{},myheader)
                .then(result=>{
                    //console.log(`MainContext: basicData: '/basicdata/getcustom'`)
                    //console.log(result.data.data[0])
                    setReloadAllTableTemplate(false)
                    const temp=createTableTemplateForPage(result.data.data)
                    setAllTableTemplate(temp)
                    //console.log(result.data.data[0])
                })
                .catch(err=>{
                    //console.log(err)
        ...
        <MainContext.Provider value={
            {
               ...
               allTableTemplate,setAllTableTemplate,
               reloadAllTableTemplate,setReloadAllTableTemplate,
